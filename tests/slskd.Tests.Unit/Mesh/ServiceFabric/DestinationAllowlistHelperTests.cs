...
public class DestinationAllowlistHelperTests
{
    ...
    public void ValidateDestinationAgainstPolicy_RegisteredServiceMatch_ReturnsTrue()
    {
        var policy = new PodPrivateServicePolicy
        {
            ...
            RegisteredServices = new List<RegisteredService> { new RegisteredService { Name = "Test DB", Host = "db.internal.company.com", Port = 5432, Protocol = "tcp" } }
        };
    ...
    {
        foreach (var allowed in policy.AllowedDestinations ?? new List<AllowedDestination>())
        {
