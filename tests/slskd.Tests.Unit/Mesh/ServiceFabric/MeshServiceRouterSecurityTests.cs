...
public class MeshServiceRouterSecurityTests
{
    ...
    {
        _logger = Mock.Of<ILogger<MeshServiceRouter>>();
        _securityLogger = Mock.Of<ILogger<SecurityEventLogger>>();
    }

    private MeshServiceRouter CreateRouter(MeshServiceFabricOptions options)
    {
        var optionsMock = new Mock<IOptions<MeshServiceFabricOptions>>();
        optionsMock.Setup(o => o.Value).Returns(options);
        return new MeshServiceRouter(_logger, optionsMock.Object);
    }
    ...
    {
        // Arrange
        var options = new MeshServiceFabricOptions
        {
    ...
    public async Task PayloadSizeLimit_RejectsOversizedPayload()
    {
        ...
        var options = new MeshServiceFabricOptions
        {
            MaxDescriptorBytes = 1024 // 1 KB (use correct property name)
        };
        var router = CreateRouter(options);
        var service = new TestService();
        router.RegisterService(service);
