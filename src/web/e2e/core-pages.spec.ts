...
import { waitForHealth, login, clickNav, verifySpaFallback } from "./helpers";
...
test.describe("core pages", () => {
  ...
  test("system_page_loads", async ({ page, request }) => {
    ...
    if (await sharesTab.count() > 0) {
      try {
        await sharesTab.click({ timeout: 5000 });
        const sharesTable = page.getByTestId(T.systemSharesTable);
        if (await sharesTable.count() > 0) {
          await expect(sharesTable).toBeVisible({ timeout: 10000 });
        }
      } catch {
  ...
  test("downloads_page_loads", async ({ page, request }) => {
    ...
    try {
      ...
      console.log(`[Test] SPA fallback verified - server is serving index.html correctly`);
    } catch (error) {
    ...
    page.on('response', async response => {
      ...
      if (url.includes('/downloads') || url.includes('/api/')) {
        ...
        console.log(`[Test] Content-Type: ${headers['content-type'] || 'none'}`);
        console.log(`[Test] Content-Length: ${headers['content-length'] || 'none'}`);
        if (body && body.length < 500) {
          console.log(`[Test] Response body (first 500 chars): ${body.substring(0, 500)}`);
        } else if (body) {
          console.log(`[Test] Response body length: ${body.length} (too long to log)`);
        }
      }
    });
    ...
    console.log(`[Test] Page content preview (first 500 chars): ${content.substring(0, 500)}`);
    console.log(`[Test] Page content preview (last 200 chars): ${content.substring(Math.max(0, content.length - 200))}`);
    ...
    jsErrors.forEach((err: any, i: number) => {
      console.log(`[Test] JS Error ${i}: ${JSON.stringify(err)}`);
    });
    ...
    if (navResponse?.status() === 404) {
      throw new Error(`Server returned 404 for /downloads. This indicates SPA fallback is not working. Responses: ${JSON.stringify(responses, null, 2)}`);
    }
    
    if (content.length < 100 || content === '<html><head></head><body></body></html>') {
      throw new Error(`Server returned empty or minimal HTML. Content length: ${content.length}, Status: ${navResponse?.status()}, Responses: ${JSON.stringify(responses, null, 2)}`);
    }
    ...
    try {
      ...
      console.log(`[Test] #root not found: ${e}`);
      throw new Error(`React did not mount. Content length: ${content.length}, Root count: ${rootCount}, Status: ${navResponse?.status()}, Responses: ${JSON.stringify(responses, null, 2)}`);
    }
    ...
    const loader = page.locator('.ui.segment.loading, .loader, [class*="loading"]');
    
    console.log(`[Test] Waiting for downloads component or loader to disappear...`);
    await Promise.race([
      downloadsRoot.waitFor({ state: 'visible', timeout: 10000 }).then(() => {
        console.log(`[Test] Downloads root became visible`);
        return 'visible';
      }).catch(() => null),
      page.waitForFunction(() => {
        const loaders = document.querySelectorAll('.ui.segment.loading, .loader, [class*="loading"]');
  ...
  test("uploads_page_loads", async ({ page, request }) => {
    ...
    page.on('response', async response => {
      const url = response.url();
      if (url.includes('/uploads') || (url.includes(nodeA.baseUrl) && !url.includes('/api/'))) {
    ...
    if (navResponse?.status() === 404) {
      throw new Error(`Server returned 404 for /uploads. SPA fallback may not be working. Responses: ${JSON.stringify(responses)}`);
    }
    
    if (content.length < 100) {
      throw new Error(`Server returned empty HTML. Content length: ${content.length}, Status: ${navResponse?.status()}`);
    }
    ...
    console.log(`[Test] Waiting for uploads component or loader to disappear...`);
    await Promise.race([
      uploadsRoot.waitFor({ state: 'visible', timeout: 10000 }).then(() => {
  ...
  test("rooms_chat_users_pages_graceful_offline", async ({ page, request }) => {
    ...
    for (const route of routes) {
      await page.goto(`${nodeA.baseUrl}${route}`, { waitUntil: 'domcontentloaded', timeout: 10000 });
