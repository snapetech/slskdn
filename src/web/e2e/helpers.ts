...
export async function login(page: Page, node: NodeCfg) {
  ...
  const networkLog: Array<{ url: string; status: number; type: string }> = [];
  page.on('response', response => {
    const url = response.url();
    if (url.includes(node.baseUrl) && !url.includes('/api/')) {
      networkLog.push({ url, status: response.status(), type: response.request().resourceType() });
  ...
  await page.goto(node.baseUrl, { waitUntil: "networkidle", timeout: 10000 });
  ...
  console.log(`[Login] After navigation - URL: ${url}, Title: ${title}, Content length: ${content.length}`);
  console.log(`[Login] Content preview (first 500 chars): ${content.substring(0, 500)}`);
  
  // Check for #root
  const rootCount = await page.locator('#root').count();
  const rootContent = await page.locator('#root').textContent().catch(() => '');
  console.log(`[Login] #root count: ${rootCount}, content length: ${rootContent.length}`);
  ...
  console.log(`[Login] React has mounted (root has children): ${hasReactContent}`);
  
  // Check network log
  console.log(`[Login] Network requests: ${networkLog.length} total`);
  const failedRequests = networkLog.filter(r => r.status !== 200);
  if (failedRequests.length > 0) {
    console.log(`[Login] Failed requests: ${JSON.stringify(failedRequests, null, 2)}`);
  }
  
  // Check console errors
  if (consoleErrors.length > 0) {
    console.log(`[Login] Console errors (${consoleErrors.length}): ${consoleErrors.join('; ')}`);
  }
  ...
  try {
    ...
    console.error(`[Login] #root not found: ${e}`);
    throw new Error(`React root not found. Content length: ${content.length}, Root count: ${rootCount}, Network requests: ${networkLog.length}, Console errors: ${consoleErrors.length}`);
  }
  ...
  try {
    ...
    console.log(`[Login] Login form not found by testid, trying placeholder...`);
    ...
    try {
      ...
      console.error(`[Login] Login form not found. Inputs: ${allInputs}, Buttons: ${allButtons}, Body text length: ${bodyText.length}`);
      if (bodyText.length < 200) {
        console.error(`[Login] Body text: ${bodyText}`);
      }
      throw new Error(`Login form not found. Content length: ${content.length}, Root count: ${rootCount}, Inputs: ${allInputs}, Console errors: ${consoleErrors.length}`);
    }
  }

  // Prefer input element over wrapper div
  const user = page.locator('input[placeholder*="Username" i]').or(page.getByTestId(T.loginUsername).locator('input')).first();
  const pass = page.locator('input[type="password"]').or(page.getByTestId(T.loginPassword).locator('input')).first();
  const submit = page.getByTestId(T.loginSubmit).or(page.locator('button:has-text("Login")')).first();
  ...
  const loginResponsePromise = page.waitForResponse(
    response => response.url().includes('/api/v0/session') && 
  ...
  if (!token) {
    ...
    const storageDebug = await page.evaluate(() => {
      return {
        sessionStorage: Object.keys(sessionStorage).map(k => ({ key: k, value: sessionStorage.getItem(k)?.substring(0, 50) })),
        localStorage: Object.keys(localStorage).map(k => ({ key: k, value: localStorage.getItem(k)?.substring(0, 50) }))
      };
    });
    console.error('[Login] Token not found after polling. Storage contents:', storageDebug);
  ...
  await Promise.race([
    page.waitForURL(url => !url.includes('/login') && url.includes(node.baseUrl), { timeout: 8000 }).catch(() => {}),
    page.waitForSelector(`[data-testid="${T.navSystem}"]`, { timeout: 8000 }).catch(() => {}),
    page.waitForLoadState('networkidle', { timeout: 5000 }).catch(() => {}) // More efficient than fixed timeout
  ]);
  ...
  try {
    ...
    console.log('[Login] Session check API call timeout (may not have been called)');
  }
  ...
  try {
    ...
    await expect(page.getByTestId(T.navSystem)).toBeVisible({ timeout: 20000 });
    ...
    const navByText = page.getByRole('link', { name: /system/i }).or(page.locator('a:has-text("System")'));
    const navByMenuItem = page.locator('.ui.menu .item[data-testid="nav-system"]');
    const navByAnyText = page.locator('text=System').filter({ has: page.locator('a, .item') });
    
    // Try waiting for any of these
    try {
      const found = navByText.or(navByMenuItem).or(navByAnyText).or(page.getByTestId(T.navSystem));
    ...
    const navByTextCount = await page.locator('text=/Search|Downloads|System/i').count();
    ...
    const menuItemsInDom = await page.evaluate(() => {
      const items = document.querySelectorAll('.ui.menu .item, [data-testid^="nav-"]');
    ...
    console.log(`[Login Debug] Menu items in DOM: ${JSON.stringify(menuItemsInDom)}`);
    ...
    if (menuItemsInDom.length > 0) {
      ...
      try {
        await page.waitForSelector('.ui.menu .item, [data-testid^="nav-"]', { state: 'visible', timeout: 5000 });
        ...
        console.log('[Login] Menu items in DOM but not visible - this may be OK');
        if (!hasLoginForm && !currentUrl.includes('/login') && token) {
          console.log('[Login] Considering login successful: no login form, token present, menu in DOM');
          return;
        }
      }
    }
    ...
    if (!hasLoginForm && !currentUrl.includes('/login') && token) {
      ...
      console.log('[Login] Login appears successful: no login form, token present, navigated away from login');
      return;
    }
    
    // Last resort: check if we're still on login page
    if (hasLoginForm || currentUrl.includes('/login')) {
      throw new Error(`Login failed - still on login page. URL: ${currentUrl}, Token: ${token ? 'present' : 'missing'}`);
    }
    
    // If we're not on login page but nav isn't visible, it might be a rendering issue
    throw new Error(`Login may have succeeded but nav elements not found. URL: ${currentUrl}, Nav testid count: ${navByTestId}, Menu items in DOM: ${menuItemsInDom.length}, Token: ${token ? 'present' : 'missing'}`);
  }
}
...
export async function clickNav(page: Page, testId: string) {
  ...
  const menuItemInLink = page.locator(`.ui.menu a:has([data-testid="${testId}"])`);
  
  // Wait for at least one to be visible
  let el = linkWithMenuItem.or(menuItemInLink).or(menuItem);
  ...
  try {
    ...
    try {
      await expect(navByMenu.first()).toBeVisible({ timeout: 8000 });
      el = navByMenu.first();
    } catch {
  ...
  try {
    await page.waitForURL(url => url !== urlBefore, { timeout: 3000 });
  } catch {
...
export async function verifySpaFallback(request: APIRequestContext, baseUrl: string) {
  ...
  console.log(`[SPA Check] Root body contains <html: ${rootBody.includes('<html')}`);
  console.log(`[SPA Check] Root body contains #root: ${rootBody.includes('id="root"') || rootBody.includes("id='root'")}`);
  ...
  if (!rootBody.includes('<html') || !rootBody.includes('root')) {
    throw new Error(`Root path does not appear to serve index.html. Body length: ${rootBody.length}, Contains <html: ${rootBody.includes('<html')}, Contains root: ${rootBody.includes('root')}`);
  }
  
  // Then verify a client-side route
  const clientRouteResponse = await request.get(`${baseUrl}/downloads`, { failOnStatusCode: false });
  const clientRouteStatus = clientRouteResponse.status();
  const clientRouteBody = await clientRouteResponse.text();
  
  console.log(`[SPA Check] Client route (/downloads) status: ${clientRouteStatus}`);
  console.log(`[SPA Check] Client route body length: ${clientRouteBody.length}`);
  console.log(`[SPA Check] Client route body contains <html: ${clientRouteBody.includes('<html')}`);
  console.log(`[SPA Check] Client route body contains #root: ${clientRouteBody.includes('id="root"') || clientRouteBody.includes("id='root'")}`);
  
  if (clientRouteStatus === 404) {
    throw new Error(`Client route /downloads returned 404. SPA fallback is not working. Root works: ${rootStatus === 200}`);
  }
  
  if (clientRouteStatus !== 200) {
    console.warn(`[SPA Check] Client route returned ${clientRouteStatus}, expected 200`);
  }
  
  if (clientRouteBody.length < 100 || !clientRouteBody.includes('<html')) {
    throw new Error(`Client route /downloads does not appear to serve index.html. Status: ${clientRouteStatus}, Body length: ${clientRouteBody.length}, Contains <html: ${clientRouteBody.includes('<html')}`);
  }
  
  return { rootOk: rootStatus === 200, clientRouteOk: clientRouteStatus === 200 };
}
