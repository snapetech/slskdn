...
3. Route imported wants to Acquisition Review by default, with an explicit option to create disabled Wishlist searches.
   This review queue is for imported, passive, and generated candidates only; active manual Search must remain direct and must not require approval before showing actionable results.
...
- 2026-04-30: T-937 implemented. Discography Coverage responses now include optional Discovery Graph priority metadata with neighborhood node/edge density, per-release gap/evidence/density scores, ranked release recommendations, and human-readable priority reasons. The scoring uses existing graph and coverage evidence only and does not browse peers, search Soulseek, start downloads, or publish graph data.
- 2026-04-30: T-931 implemented. It adds versioned salted MusicBrainz library Bloom snapshots for local recording/release identifiers, preview metadata and privacy notes, inbound snapshot comparison against local cached MusicBrainz candidates, and review-only Wishlist promotion for likely missing tracks. Snapshots do not expose filenames, paths, file hashes, or exact identifiers, and diff suggestions remain probabilistic.
...
- 2026-04-30: T-932 persistence follow-up implemented. Artist release radar subscriptions, muted release groups, seen-observation keys, and notifications now persist to an atomic JSON state file under the app directory and reload on service startup, preserving restart-safe notification history and duplicate suppression.
- 2026-04-30: T-932 routing follow-up implemented. Artist release radar notifications can now be routed through PodCore to explicitly selected safe peers, with signed local route envelopes, persisted route attempts, API dispatch/history endpoints, and safe opaque route metadata validation. Routing remains user-initiated and does not automatically publish observations, search Soulseek, browse peers, download, or mutate files.
...
- 2026-04-30: T-933 graph/handoff follow-up implemented. Federated taste recommendations can now include optional Discovery Graph evidence and proximity scoring, create review-only Wishlist seeds, create artist release radar subscriptions when artist MBIDs are present, and return Discovery Graph preview summaries. These handoffs stay explicit and do not start Soulseek searches, browse peers, download, publish, or mutate files.
...
- 2026-04-30: T-934 governance proposal follow-up implemented. Subject-index revisions can now be proposed as realm governance documents, reviewed by trusted governance roots, accepted into the resolver only after explicit approval, or rejected while preserving review provenance.
- 2026-05-01: T-934 conflict-report follow-up implemented. Accepted realm subject indexes can now produce deterministic conflict reports for external-id disagreements, one recording mapped to multiple subjects, conflicting WorkRef identity, and aliases mapped to multiple subjects; the read-only API exposes accepted indexes, recording resolutions, and conflict reports with realm/index/revision provenance.
- 2026-05-01: T-934 authority-decision follow-up implemented. Accepted realm subject-index authorities can now be locally disabled or re-enabled through backend API decisions; disabled authorities are excluded from recording resolution and conflict reports while accepted indexes and governance documents remain intact.
- 2026-05-01: T-934 persistence follow-up implemented. Accepted realm subject indexes, governance proposal review state, and local authority decisions now persist to an atomic app-dir JSON state file and reload on service startup, preserving resolver data and disabled authority preferences across restarts without publishing indexes or contacting peers.
...
- 2026-04-30: T-935 trust-scoped routing follow-up implemented. Stored signed MusicBrainz overlay edits can now be routed through PodCore to explicitly selected safe peers only, with route attempts recording target, routed, and failed peer IDs while preserving provenance and avoiding automatic publication.
- 2026-04-30: T-935 manual export follow-up implemented. Stored overlay edits can now produce manual upstream MusicBrainz submission review packages with target, proposed change, and evidence, and users can record explicit local export approval without auto-submitting edits upstream or mutating cached MusicBrainz payloads.
- 2026-05-01: T-935 persistence follow-up implemented. Signed overlay edits, selected-peer route attempts, and manual upstream export approvals persist to an atomic JSON state file and reload on service startup without publishing, submitting upstream edits, searching, browsing peers, downloading, or mutating cached MusicBrainz payloads.
...
- 2026-04-30: T-936 routing follow-up implemented. Quarantine Jury requests can now be routed through PodCore to explicitly selected safe jurors only, with persisted route attempts recording routed and failed jurors; the route payload carries minimal safe evidence and does not attach raw files, expand audiences, or release quarantined content.
- 2026-04-30: T-936 manual acceptance follow-up implemented. The API now exposes a review projection with request evidence, signed verdicts, route attempts, aggregate recommendation, and acceptance status, plus a persisted manual accept endpoint that only records user acceptance of release-candidate supermajorities and does not mutate quarantine enforcement.
- 2026-05-01: T-936 audit-report follow-up implemented. The backend now returns a read-only Quarantine Jury audit report with accepted release candidates, pending release-candidate acceptances, manual-review requests, uphold-quarantine recommendations, stale requests, routed requests, and failed route attempts without changing local quarantine state.
- 2026-05-01: T-936 release-package follow-up implemented. Locally accepted release-candidate recommendations can now produce read-only release evidence packages with request evidence, signed juror verdicts, route attempts, acceptance snapshots, current aggregate state, and drift warnings without changing quarantine enforcement, moving files, publishing decisions, searching, browsing peers, or downloading.
...
- 2026-04-30: T-939 non-Spotify provider expansion implemented. Apple Music/iTunes URLs with numeric ids resolve through public lookup, ListenBrainz user URLs import public listens, and YouTube/Bandcamp/Last.fm/Apple pages fall back to Open Graph/title metadata. These imports remain review-first, bounded by the import limit, and do not start Soulseek searches, peer browses, or downloads.
- 2026-04-30: T-939 API-key provider expansion implemented. Optional `integrations.youtube.api_key` enables YouTube playlist expansion through the Data API, and optional `integrations.lastfm.api_key` enables Last.fm loved/recent/top track imports. Both remain explicit import previews bounded by the import limit and do not start Soulseek searches, peer browses, or downloads.
- 2026-04-30: T-939 Web UI settings follow-up implemented. System Integrations now exposes source-feed provider settings for Spotify, YouTube, and Last.fm with on/off toggles, masked credential entry, validation warnings, and tooltip-backed runtime apply/reset controls.
- 2026-05-01: T-939 history/audit follow-up implemented. Source-feed preview runs now persist bounded app-dir history entries with provider/source metadata, source fingerprints, safe source previews, request/result counts, network-request counts, skipped-row samples, and suggestion samples; authenticated list/detail endpoints expose the audit trail without storing provider bearer tokens or starting any Soulseek search, browse, or download.
