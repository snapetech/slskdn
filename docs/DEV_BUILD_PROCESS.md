...
- **Stable submission**: Main releases auto-submit a PR to `microsoft/winget-pkgs` via `wingetcreate --submit`; the job fails if `WINGETCREATE_GITHUB_TOKEN` is missing so skipped submissions cannot look successful
