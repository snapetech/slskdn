...
## 2026-03-23 13:34 - Relay upload failure callback fix
...
- Continued into shared timer fanout after the callback-observer fixes.
- Fixed `Clock.Fire(...)` so one throwing subscriber no longer aborts the rest of the scheduled handlers for that tick.
- Added focused `ClockTests` coverage and kept the watchdog/VPN lifecycle regressions green in the same slice.
- Validation passed:
...
## 2026-03-23 11:39 CST

- Fixed a duplicate-greeting race in `DhtPeerGreetingService`.
- Neighbor greeting hooks now reserve the peer in `_greetedPeers` before launching the fire-and-forget private-message send, and they roll the reservation back if the send cannot start or fails.
- Added focused regression coverage in `tests/slskd.Tests.Unit/DhtRendezvous/DhtPeerGreetingServiceTests.cs`.
- Validation: `dotnet build src/slskd/slskd.csproj -c Release -v minimal` passed with `0 warnings / 0 errors`; focused `DhtPeerGreetingServiceTests` slice passed.
...
- Continued the same ownership/lifecycle sweep into the DHT service layer instead of treating DHT and registry hooks as self-cleaning background-service details.
- `DhtRendezvousService` now starts the `BackgroundService` loop only once, initializes `DhtEngine`/listener through local ownership until startup succeeds, detaches `PeersFound`/`StateChanged` handlers before teardown, and releases the live listener/engine fields through stable local captures during stop/dispose.
- `DhtPeerGreetingService` now attaches neighbor-registry subscriptions from `StartAsync()`, keeps the hosted service alive until cancellation, and detaches the registry hooks on both `StopAsync()` and `Dispose()` so repeated starts and direct disposal no longer leave duplicate or stale handlers behind.
- Extended `tests/slskd.Tests.Unit/Core/HostedServiceLifecycleTests.cs` and `tests/slskd.Tests.Unit/DhtRendezvous/DhtPeerGreetingServiceTests.cs` with focused lifecycle coverage around DHT engine unsubscription, canceled initialization teardown, duplicate-start subscription guards, and dispose-time registry detach.
...
- Continued the same ownership pattern into the signal-system channel layer instead of stopping after the DHT hosted-service fixes.
- `ISignalChannelHandler` now exposes disposal, `MeshSignalChannelHandler` and `BtExtensionSignalChannelHandler` now detach from their underlying sender events on dispose, and `SignalBus.Dispose()` now tears down owned registered handlers as part of bus shutdown.
...
- Continued the same ownership-contract pattern across the remaining DI-facing singleton interfaces with already-disposable implementations instead of stopping at the mesh-only subset.
- `IGossipService`, `IRealmAwareGossipService`, `IGovernanceClient`, `IRealmAwareGovernanceClient`, `ISignalBus`, `IActivityPubKeyStore`, `IAnonymityTransportSelector`, `ILanDiscoveryService`, `IUploadService`, `IUploadQueue`, and `IUploadGovernor` now all expose disposal so their public contracts match the concrete singletons already holding subscriptions, transports, timers, caches, semaphores, and child helper ownership.
- Kept the sweep bounded to actual singleton service handles resolved from DI, not every disposable helper or transient abstraction in the tree.
- Validation: `dotnet test tests/slskd.Tests.Unit/slskd.Tests.Unit.csproj --no-build --filter "FullyQualifiedName~SignalBusTests|FullyQualifiedName~LanDiscoveryServiceTests|FullyQualifiedName~ActivityPubKeyStoreTests|FullyQualifiedName~UploadGovernorTests|FullyQualifiedName~UploadQueueTests|FullyQualifiedName~UploadServiceLifecycleTests|FullyQualifiedName~RealmAwareGovernanceClientTests|FullyQualifiedName~RealmAwareGossipServiceTests"` passed (`100/100`), `dotnet build tests/slskd.Tests.Unit/slskd.Tests.Unit.csproj -v minimal -clp:ErrorsOnly` passed (`0 errors`), `dotnet build src/slskd/slskd.csproj -c Release -v minimal` passed (`0 warnings / 0 errors`), and `bash ./bin/lint` passed.

## 2026-03-23 19:10 CST

- Continued the singleton disposal-contract sweep into the privacy/moderation/discovery layer instead of leaving the interface expansion half-finished after the first compile cleanups.
- `IDhtRateLimiter`, `IScenePubSubService`, `IDnsSecurityService`, `ICoverTrafficGenerator`, `IExternalModerationClient`, `ILlmModerationProvider`, `ISourceDiscoveryService`, and both `IPrivacyLayer` interfaces now expose disposal to match their concrete owners.
- Fixed the follow-on ownership bug in both privacy-layer implementations: they now fail closed after disposal, and the mesh privacy layer now disposes replaced helper components during `UpdateConfiguration(...)` instead of orphaning the previous padder/jitter/batcher/cover-traffic instances.
- Also fixed the underlying mesh privacy helper family (`BucketPadder`, `RandomJitterObfuscator`, `TimedBatcher`, `CoverTrafficGenerator`) so they dispose owned RNG/semaphore state explicitly and throw `ObjectDisposedException` after teardown, making replacement/disposal regressions observable instead of silent.
- Added focused regressions in `tests/slskd.Tests.Unit/Mesh/Privacy/PrivacyLayerIntegrationTests.cs` for replace/dispose behavior and updated the affected scene-service test stub to satisfy the widened `IScenePubSubService` contract.
- Validation: `dotnet build tests/slskd.Tests.Unit/slskd.Tests.Unit.csproj -v minimal -clp:ErrorsOnly` passed (`0 errors`), `dotnet test tests/slskd.Tests.Unit/slskd.Tests.Unit.csproj --no-build --filter "FullyQualifiedName~PrivacyLayerIntegrationTests|FullyQualifiedName~OverlayPrivacyIntegrationTests|FullyQualifiedName~LlmModerationTests|FullyQualifiedName~LocalExternalModerationClientTests|FullyQualifiedName~RemoteExternalModerationClientTests|FullyQualifiedName~SceneServicesTests"` passed (`52/52`), `dotnet build src/slskd/slskd.csproj -c Release -v minimal` passed (`0 warnings / 0 errors`), and `bash ./bin/lint` passed.

## 2026-03-23 19:43 CST

- Continued the broader ownership pass into operation-tracking dictionaries rather than stopping at singleton interface exposure.
- Fixed `SearchService` so tracked search `CancellationTokenSource` instances are retired through a shared helper on completion, cancellation, bridged-search teardown, and service disposal; `ISearchService` now exposes disposal to match the singleton’s owned in-flight search state.
- Fixed `MeshTransferService` so tracked per-transfer cancellation sources and progress subjects are retired on terminal paths and service disposal; `IMeshTransferService` now exposes disposal to match those owned trackers.
- Added focused lifecycle regressions in `tests/slskd.Tests.Unit/Search/SearchServiceLifecycleTests.cs` and `tests/slskd.Tests.Unit/VirtualSoulfind/DisasterMode/MeshTransferServiceLifecycleTests.cs` covering tracked CTS disposal, terminal cleanup, and dispose-time publisher completion.
- Validation: `dotnet build tests/slskd.Tests.Unit/slskd.Tests.Unit.csproj -v minimal -clp:ErrorsOnly` passed (`0 errors`), `dotnet test tests/slskd.Tests.Unit/slskd.Tests.Unit.csproj --filter "FullyQualifiedName~SearchServiceLifecycleTests|FullyQualifiedName~MeshTransferServiceTests|FullyQualifiedName~MeshTransferServiceLifecycleTests"` passed (`7/7`), `dotnet build src/slskd/slskd.csproj -c Release -v minimal` passed (`0 warnings / 0 errors`), and `bash ./bin/lint` passed.
## 2026-03-28 13:40 CST

- Resolved GitHub issues `#164` and `#165` in the startup/runtime path cluster.
- `Program` now constructs the DI-time placeholder `SoulseekClient` with listener and distributed-network behavior disabled until `Application.InitializeApplicationAsync(...)` applies the real startup configuration, which closes the startup race behind the unobserved `Not listening...` / `Connection refused` noise seen in Docker.
- `InstallShutdownTelemetry()` now downgrades those exact known-benign upstream listener/connect unobserved exceptions from fake-fatal shutdown telemetry to warnings while still observing the task and preserving fatal logging for unexpected exceptions.
- Added `ProgramPathNormalizationTests` coverage proving the inert initial `SoulseekClientOptions` shape and the benign-unobserved-exception classifier behavior, and documented the pattern in `adr-0001-known-gotchas.md` (`0xF07`).
- Validation: `dotnet test tests/slskd.Tests.Unit/slskd.Tests.Unit.csproj --filter "FullyQualifiedName~ProgramPathNormalizationTests|FullyQualifiedName~ConnectionWatchdogTests" -v minimal` passed (`12/12`), `dotnet build src/slskd/slskd.csproj -c Release -v minimal -clp:ErrorsOnly` passed (`0 warnings / 0 errors`), full `dotnet test` passed (`3671/3671` across unit/integration/API suites), and `bash ./bin/lint` passed.
