...
- [ ] **Security follow-up: close remaining CodeQL alert clusters**: As of 2026-03-21 there are 24 open code-scanning alerts after the first narrowing/fix pass. Remaining clusters are `cs/user-controlled-bypass` in relay/session/Tor authorization flow, `cs/resource-injection` in `SqliteShareRepository`, `cs/sql-injection` in `HashDbOptimizationService`, `cs/cleartext-storage-of-sensitive-information` in `Program` / `AsymmetricDisclosure`, and `cs/web/cookie-httponly-not-set` for the antiforgery cookie. Triage each cluster, fix true positives, and document justified dismissals so the alert set reaches a stable baseline instead of drifting.
