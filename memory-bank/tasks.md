...
- [x] **Packaging follow-up: automate the NixOS VM smoke test**: Added [run-nix-package-smoke.sh](/home/keith/Documents/code/slskdn/packaging/scripts/run-nix-package-smoke.sh) as a reusable CI-friendly harness that builds `.#default`, launches the packaged `bin/slskd`, and evaluates a minimal `services.slskd` NixOS configuration with the required `domain`, `environmentFile`, and `settings.shares.directories = [ ]` contract. Wired it into [ci.yml](/home/keith/Documents/code/slskdn/.github/workflows/ci.yml) and [build-on-tag.yml](/home/keith/Documents/code/slskdn/.github/workflows/build-on-tag.yml), including a post-metadata-update smoke in the main-channel tag workflow. Validated locally with `bash packaging/scripts/run-nix-package-smoke.sh` and `bash ./bin/lint` on 2026-03-22.
...
- [x] **Release regression follow-up: add a subpath-hosted web smoke test**: Added [smoke-subpath-build.mjs](/home/keith/Documents/code/slskdn/src/web/scripts/smoke-subpath-build.mjs), wired it into [run-release-gate.sh](/home/keith/Documents/code/slskdn/packaging/scripts/run-release-gate.sh), and extended the release docs so the minimum gate now serves the built UI under `/slskd/` and verifies the relative asset URLs actually load from that mount point. Validated locally with `bash packaging/scripts/run-release-gate.sh` and `bash ./bin/lint` on 2026-03-22.
- [x] **Release hardening follow-up: add release-surface integration smoke and checklist**: Added `packaging/scripts/run-release-integration-smoke.sh`, wired it into `packaging/scripts/run-release-gate.sh`, documented the operator-facing checklist in `docs/dev/release-checklist.md`, and extended `docs/dev/testing-policy.md` so the minimum release gate now covers targeted integration smoke (`LoadTests`, `DisasterModeIntegrationTests`, `SoulbeetAdvancedModeTests`, `CanonicalSelectionTests`, `LibraryHealthTests`). Validated locally with `bash packaging/scripts/run-release-gate.sh` and `bash ./bin/lint` passing on 2026-03-22.
...
- [ ] **Controller contract cleanup follow-up**: After the ongoing broad controller normalization/error-contract sweep, run `dotnet test` and `./bin/lint` from the cleaned head and fix any regressions before the next release tag.
