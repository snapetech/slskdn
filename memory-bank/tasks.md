...
- [x] **bug**: Triage issue `#209` on `kspls0` and quiet app-side live noise.
 ...
 - Priority: P1
 - Notes: `kspls0` is already on `slskdn-bin 0.24.5.slskdn.174-1`, active under systemd with `NRestarts=0`, Soulseek logged in, shares ready, DHT ready at `250` nodes, and overlay listening on `50305`. The mesh population is thin/unreliable rather than absent: `/api/v0/overlay/stats` showed `1` active verified mesh connection out of `636` failed outbound overlay attempts, mostly timeout/no-route/TLS EOF remote endpoint churn. Normal Soulseek downloads are working, but two manual API searches (`radiohead`, `beatles`) returned zero responses while auto-replace was repeatedly consuming the user/API search safety budget. Fixed app-side defects found in the same pass: common remote transfer rejections now classify as expected peer policy instead of fake fatal unobserved tasks, circuit maintenance no longer runs automatic placeholder circuit-building probes against live peers, background auto-replace uses an `auto-replace` safety source instead of the `user` bucket, and search completion logs include source-specific response counts for the next live retest. Gotchas are documented in ADR-0001.
